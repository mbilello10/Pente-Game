				...
				for (size_t j = 0; j < moves8.size(); j++) {
					...
					for (int i = 1; i < END_SPOT_4; i++) {
						...
						if (board[row + i * y_change][col + i * x_change] != BLACK) { 
							...
							//special case win spot that could lead to 5 eg B_BBB
							...
							//special case of win spot that could lead to 5 eg BB_BB
							...
							if (i == END_SPOT_4 - 1 && !is_3) { // don't run through again once 3 in row found
								...
								//break if deadend. e.g current open spot looks like W_BBBW
								if(board[row + i *y_change][col + i * x_change] == WHITE && (row - y_change) < SIZE && (col - x_change) < SIZE && (row - y_change) >= 0 && (col - x_change) >= 0 && board[row - y_change][col - x_change] == WHITE) break;
								...
								if (board[row + i * y_change][col + i * x_change] == WHITE) break;
								is_1 = true;
								temp_1_y = row;
								temp_1_x = col;
								break;
			...
			if (board[row][col] == OPEN) {
				...
				for (size_t j = 0; j < moves8.size(); j++) {
					...
					for (int i = 1; i < END_SPOT_4; i++) {
						...
						//special case block middle spot that could lead to 5 eg W_WWW
							...
							//special case of block middle spot that could lead to 5 eg WW_WW
							...
							// run this loop if spot looks like _WWW_
							...
							else if (i == END_SPOT_4 - 2 && !is_block_2) { //don't run through once 2 in row found
				...
				for (size_t j = 0; j < moves8.size(); j++) {
					...
					for (int i = 1; i < END_SPOT_3; i++) {
						...
						if (i == END_SPOT_3 - 1) {
							...
							if ((board[y_3][x_3] == BLACK)  && (board[y_1][x_1] == board[y_2][x_2]) && (board[y_1][x_1] == WHITE)) {
								y = row;
								x = col;
								return;
							}
...
// to block 3 in a row of opposite color eg BWWW
	if (is_block_3) {
		y = block_3_y;
		x = block_3_x;
		return;
...
// to block 2 in a row of opposite color eg BWW
	if (is_block_2) {
		y = block_2_y;
		x = block_2_x;
		return;
